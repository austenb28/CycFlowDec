...
Runs percolating cyclic flow decomposition on `F`. Stores cycles as keys in the dictionary `CycFlowDec.cycles`, whose items are the corresponding cycle's unscaled weight. Cycles are defined as tuples, with the first node in the tuple being the lowest index node in the cycle.
